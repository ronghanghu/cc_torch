...
__global__ void init_labeling(int32_t *label, const uint32_t W,
                              const uint32_t H) {
...
__global__ void merge(uint8_t *img, int32_t *label, const uint32_t W,
                      const uint32_t H) {
    const uint32_t row = (blockIdx.y * blockDim.y + threadIdx.y) * 2;
    const uint32_t col = (blockIdx.x * blockDim.x + threadIdx.x) * 2;
    const uint32_t idx = row * W + col;
    ...
    if (P > 0) {
        ...
        if ((hasBit(P, 1) && img[idx - W]) ||
            (hasBit(P, 2) && img[idx - W + 1]))
            union_(label, idx, idx - 2 * W); // top bottom block

        if (hasBit(P, 3) && img[idx + 2 - W])
            union_(label, idx, idx - 2 * W + 2); // top right block

        if ((hasBit(P, 4) && img[idx - 1]) ||
            (hasBit(P, 8) && img[idx + W - 1]))
...
__global__ void init_counting(const int32_t *label, int32_t *count_init,
                              const int32_t W, const int32_t H) {
...
__global__ void final_counting(const int32_t *label, const int32_t *count_init,
                               int32_t *count_final, const int32_t W,
                               const int32_t H) {
    const uint32_t row = (blockIdx.y * blockDim.y + threadIdx.y);
    const uint32_t col = (blockIdx.x * blockDim.x + threadIdx.x);
    const uint32_t idx = row * W + col;
...
connected_componnets_labeling_2d(const torch::Tensor &inputs, bool get_counts) {
    AT_ASSERTM(inputs.is_cuda(), "inputs must be a CUDA tensor");
    AT_ASSERTM(inputs.ndimension() == 4, "inputs must be [N, 1, H, W] shape");
    AT_ASSERTM(inputs.scalar_type() == torch::kUInt8,
               "inputs must be a uint8 type");
    ...
    dim3 grid = dim3(((W + 1) / 2 + BLOCK_COLS - 1) / BLOCK_COLS,
    ...
    for (int n = 0; n < N; n++) {
        ...
        if (get_counts) {
            cc2d::init_counting<<<grid_count, block_count, 0, stream>>>(
                ...
                counts_init.data_ptr<int32_t>() + offset, W, H);
            cc2d::final_counting<<<grid_count, block_count, 0, stream>>>(
                labels.data_ptr<int32_t>() + offset,
                counts_init.data_ptr<int32_t>() + offset,
                counts_final.data_ptr<int32_t>() + offset, W, H);
        }
    }
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("cc_2d", &connected_componnets_labeling_2d,
