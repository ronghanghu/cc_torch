...
__global__ void final_counting(const int32_t *label, const int32_t *count_init, int32_t *count_final, const int32_t W, const int32_t H)
{
    const uint32_t row = (blockIdx.y * blockDim.y + threadIdx.y);
    const uint32_t col = (blockIdx.x * blockDim.x + threadIdx.x);
    const uint32_t idx = row * W + col;
...
std::vector<torch::Tensor> connected_componnets_labeling_2d(const torch::Tensor &inputs, bool get_counts) {
    AT_ASSERTM(inputs.is_cuda(), "inputs must be a CUDA tensor");
    AT_ASSERTM(inputs.ndimension() == 4, "inputs must be [N, 1, H, W] shape");
    AT_ASSERTM(inputs.scalar_type() == torch::kUInt8, "inputs must be a uint8 type");
    ...
    auto label_options = torch::TensorOptions().dtype(torch::kInt32).device(inputs.device());
    ...
    dim3 grid_count = dim3((W + BLOCK_COLS) / BLOCK_COLS, (H + BLOCK_ROWS) / BLOCK_ROWS);
